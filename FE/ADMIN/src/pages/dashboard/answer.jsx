...
function Answer() {
    ...
    const onFinish2 = async (values) => {
        ...
        if (size < 4) {
         const res =    await addAnswer(quesId, cateId, values)
            if (res?.status === 200) {
    ...
    const onFinish3 = async (data) => {
        ...
        const dataupdate = {
         ...
         const res =     await  updateAnswerA(updateAnswer?.id , quesId , cateId , dataupdate   );
         if (res?.status === 200) {
            toast.success("Adjust Answer successfully");
        } else {
            toast.error("Adjust Answer failed")
        }
        await handleGetAnswer()
    }
      const handleDelete  = async(data )=>{
      const res =   await deleteAnswer(data?.id  , quesId , cateId);
      if (res?.status === 200) {
        toast.success("Delete Answer successfully");
    } else {
        toast.error("Delete Answer failed")
    }
        await handleGetAnswer()
      }
    return <>
