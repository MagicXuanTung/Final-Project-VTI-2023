...
function Topic() {
    ...
    const handleDeleteTopic = async (data) => {
        ...
        if (res?.status === 200) {
            toast.success("delet thanh cong");
        } else {
            toast.dark(" delete that bai np")
        }
        handleGetAll();
    }
    ...
    const onFinish1 = async (values) => {
        console.log("sajdhgasd", [values?.quesID], topicUpdate);

        const res = await updateTopic(topicUpdate?.id, [values?.quesID]);
        if (res?.status === 200) {
            toast.success("them question thanh cong");
        } else {
            toast.dark("  that bai np")
        }
        form1.resetFields();
        handleGetAll();
    }
    ...
    console.log(" sahdahsd", modalAboutData);
    return <>
        <ToastContainer
            position="top-right"
            autoClose={5000}
        ...
        <div className="m-sm-30">
            ...
            <div style={{ position: "relative", marginLeft: 20, height: 200, width: "1000px", backgroundColor: "#FFA07A", borderRadius: 5 }} >
                <div style={{ margin: "30px 20px 20px 20px ", position: "absolute" }}>
                    <Title style={{ marginBottom: 20 }} level={3}> Them Topic</Title>
                    <Form form={form} onFinish={onFinish}>
                        ...
                        <Button htmlType="submit" style={{ marginLeft: 450, height: 40, width: 120, backgroundColor: '#FFD700', borderColor: '#FFD700', color: "#fff", fontSize: 16 }} >them</Button>
                    </Form>

                </div>
            </div>
            <Title level={3} style={{ marginTop: 20 }}> danh sach Topic</Title>
                        ...
                        return (
                            <div key={item?.id} style={{ backgroundColor: '#FF9933', height: 100, width: "100%", marginBottom: 30, borderRadius: 5 }} >
                                <div className="content" style={{ display: "flex ", alignItems: "center", paddingTop: 10 }}>
                                    ...
                                    <Button onClick={() => handleAbout(item)} style={{ marginLeft: 2, backgroundColor: "#BC8F8F", borderColor: '#BC8F8F', color: "#fff" }}> about</Button>
                                    <Button onClick={() => handleUpdate(item)} style={{ marginLeft: 2, backgroundColor: "#BC8F8F", borderColor: '#BC8F8F', color: "#fff" }}> them question</Button>
        ...
        <Modal centered open={shouldOpenModal} footer={null} onCancel={handleCancel} okButtonProps={okButtonProps}>
            <Title level={3}> them question cho Topic</Title>
        ...
        <Modal centered open={shouldModalAbout} footer={null} onCancel={handleCancel} okButtonProps={okButtonProps} >
            <Title level={3}> danh sach question cua topic </Title>
            {/* {modalAboutData !== null ? (<> {modalAboutData?.questionListID.map((item) => {
